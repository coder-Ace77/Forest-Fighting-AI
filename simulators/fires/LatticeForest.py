...
class LatticeForest(Simulator):
    ...
    def __init__(
        ...
        self.fires = (
            []
        )  # list containing (row, col) positions corresponding to Trees on fire
    ...
    def dense_state(self):
        ...
        return np.array(
            [
                [self.group[(r, c)].state for c in range(self.dims[1])]
                for r in range(self.dims[0])
            ]
        )
    ...
    def update(self, control=None):
        ...
        for f in self.fires:
            for fn in self.group[f].neighbors:
                if fn not in checked and self.group[fn].is_healthy(
                    self.group[fn].state
                ):
                    self.early_end = False
        ...
        self.fires = [
            f for f in self.fires if self.group[f].is_on_fire(self.group[f].state)
        ]
    ...
    def getState(self):
        ...
        for r in range(self.dims[0]):
            temp = []
            for c in range(self.dims[1]):
                # print(self.group[(r, c)].state, end=" ")
                temp.append(self.group[(r, c)].state)
            lattice.append(temp)

        return lattice

    # returns in range(0-1)
    def getfracHealthy(self):
        state = self.getState()
        count_healthy = 0
        total_count = self.dims[0] * self.dims[1] + 1
        for i in range(self.dims[0]):
            for j in range(self.dims[1]):
                if state[i][j] == 0:
                    count_healthy += 1

        return count_healthy / total_count
